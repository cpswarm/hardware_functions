...
//**********************************************************************************************************
bool isAtAltitude(double target_altitude, double tolerance) {
	double distance = abs(target_altitude - local_position.pose.position.z);
...
int main(int argc, char **argv) {
	...
	nh.param(ros::this_node::getName() + "/offset_x", offset_x, 0.0);
	//Get takeoff offset y direction
	nh.param(ros::this_node::getName() + "/offset_y", offset_y, 0.0);
	...
	server.start();
	ROS_INFO("TAKEOFF - TakeOff action available");
	ros::spin();
